...
public class PhyMLv3AminoAcidRunEstimator extends AminoAcidRunEstimator {
	...
	public boolean runEstimator() 
		...
		this.workAlignment = TemporaryFileManager.getInstance()
		.getAlignmentFilename(Thread.currentThread()); 
		...
		case ApplicationGlobals.OPTIMIZE_FIXED_BIONJ:
			if(TemporaryFileManager.getInstance()
					.getTreeFilename(Thread.currentThread())!= null) {
				tr = TemporaryFileManager.getInstance()
					.getTreeFilename(Thread.currentThread());
				topo = "l";
			} else {
				topo = "r";
			}
...
//					str[6] = str[7] = "";
			...
			FileOutputStream tmp = new FileOutputStream(
					TemporaryFileManager.getInstance()
			...
			if(exitVal != 0) {
				...
				String line;
				try {
					FileReader input = new FileReader(TemporaryFileManager.getInstance()
							.getLogFilename(Thread.currentThread()));
					...
					err.println("Unable to read the log file: " 
							+ TemporaryFileManager.getInstance()
							.getLogFilename(Thread.currentThread()));
				}
			}
				...
				if(line.length()>0)
					if(line.startsWith(". Model of amino acids substitution")) {
						...
						if(Utilities.lastToken(line).equals("Yes")) {
							line = br.readLine();
							if(verbose) System.out.println("[DEBUG] PHYML: " + line);
							if(model.getNumberOfTransitionCategories() != Integer.parseInt(Utilities.lastToken(line))) {
								String errorMsg = "There was some error in the number of transition categories: " 
									+ model.getNumberOfTransitionCategories()+ " vs " + Integer.parseInt(Utilities.lastToken(line));
								err.println(errorMsg);
							...
							line = br.readLine();
							if(verbose) out.println("[DEBUG] PHYML: " + line);
	...
	protected boolean deleteTemporaryFiles() {
		...
		f.delete();
		f = new File(TemporaryFileManager.getInstance()
				.getLogFilename(Thread.currentThread()));
		f.delete();
		return true;
	}
